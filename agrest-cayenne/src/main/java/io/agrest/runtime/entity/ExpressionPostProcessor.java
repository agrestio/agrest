...
public class ExpressionPostProcessor implements IExpressionPostProcessor<Expression> {

    private IPathDescriptorManager<Expression> pathCache;
    ...
    private Expression validateAndCleanup(AgEntity<?> entity, Expression exp) {
		...
		if (exp instanceof ASTObjPath) {
			exp = pathCache.getPathDescriptor(entity,  exp).getPathExp();
		}
