 ...
 * @since 2.7
 */
public abstract class CayenneAssembleQueryStage implements Processor<SelectContext<?, ?>> {
...
//    }
    ...
    <T> SelectQuery<T> buildQuery(SelectContext<T, Expression> context) {

        ResourceEntity<T, Expression> entity = context.getEntity();
        ...
        if (context.getParent() != null) {
            Expression qualifier = Util.qualifierByParent(context.getParent(), entityResolver);
            query.andQualifier(qualifier);
        }

        if (entity.getQualifier() != null) {
            Expression expression = entity.isQualified()
                    ? entity.getQualifier()
                    : entity.qualify(
    ...
    <T> SelectQuery<T> basicSelect(SelectContext<T, Expression> context) {
