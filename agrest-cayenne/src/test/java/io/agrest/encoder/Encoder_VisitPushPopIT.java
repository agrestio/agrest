...
public class Encoder_VisitPushPopIT extends EncoderITBase {
	...
	public void testVisit_Tree() {
		...
		EncoderITBase.DB.insert("e2", "id, name", "1, 'xxx'");
		EncoderITBase.DB.insert("e2", "id, name", "2, 'yyy'");
		EncoderITBase.DB.insert("e2", "id, name", "3, 'zzz'");

		EncoderITBase.DB.insert("e5", "id, name", "1, 'xxx'");
		EncoderITBase.DB.insert("e5", "id, name", "2, 'yyy'");

		EncoderITBase.DB.insert("e3", "id, e2_id, e5_id, name", "7, 2, 1, 'zzz'");
		EncoderITBase.DB.insert("e3", "id, e2_id, e5_id, name", "8, 1, 1, 'yyy'");
		EncoderITBase.DB.insert("e3", "id, e2_id, e5_id, name", "9, 1, 2, 'zzz'");

		MultivaluedHashMap<String, String> params = new MultivaluedHashMap<>();
		params.putSingle("include", "{\"path\":\"e3s\",\"sort\":\"id\"}");
		params.putSingle("include", "e3s.e5");

		UriInfo mockUri = Mockito.mock(UriInfo.class);
		Mockito.when(mockUri.getQueryParameters()).thenReturn(params);
