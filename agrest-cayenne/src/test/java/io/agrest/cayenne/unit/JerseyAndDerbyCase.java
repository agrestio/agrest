...
public abstract class JerseyAndDerbyCase {
    ...
    protected static void startTestRuntime(BiFunction<AgBuilder, ICayennePersister, AgBuilder> agCustomizer, Class<?>... resources) {
        ...
        TEST_RUNTIME = TEST_FACTORY.app("-s", "-c", "classpath:io/agrest/cayenne/unit/server.yml")
                .autoLoadModules()
    ...
    public static class AgModule implements BQModule {
        ...
        AgRuntime createRuntime(ServerRuntime runtime) {
            ...
            AgCayenneBuilder cayenneBuilder = AgCayenneBuilder.builder().persister(persister);
            AgBuilder agBuilder = new AgBuilder().module(cayenneBuilder.build());
            return agCustomizer.apply(agBuilder, persister).build();
        }
    }
}
