...
public class EntityPathCacheTest extends TestWithCayenneMapping {
	...
	public void testMultiColumnId() {
		...
		PathDescriptor<ASTObjPath> pdName = cache.getPathDescriptor(new ASTObjPath("name"));
		...
		PathDescriptor<ASTObjPath> pdId1 = cache.getPathDescriptor(new ASTObjPath("id1"));
		assertNotNull(pdId1);
		...
		PathDescriptor<ASTObjPath> pdId2 = cache.getPathDescriptor(new ASTObjPath("id2"));
		assertNotNull(pdId2);
	...
	public void testGetPathDescriptor_Attribute() {
		...
		PathDescriptor<ASTObjPath> pd = cache.getPathDescriptor(new ASTObjPath("name"));
		assertNotNull(pd);
	...
	public void testGetPathDescriptor_Relationship() {
		...
		PathDescriptor<ASTObjPath> pd = cache.getPathDescriptor(new ASTObjPath("e2"));
		assertNotNull(pd);
	...
	public void testGetPathDescriptor_RelatedAttribute() {
		EntityPathCache cache = new EntityPathCache(getAgEntity(E3.class));
		PathDescriptor<ASTObjPath> pd = cache.getPathDescriptor(new ASTObjPath("e2.name"));
		assertNotNull(pd);
		assertTrue(pd.isAttribute());
		assertEquals(String.class, pd.getType());
