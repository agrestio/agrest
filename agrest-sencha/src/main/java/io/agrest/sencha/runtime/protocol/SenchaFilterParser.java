    ...
    public List<Filter> fromString(String filtersJson) {
        ...
        for (JsonNode filterNode : rootNode) {
            ...
            if (propertyNode == null) {
                throw new AgRESTException(Response.Status.BAD_REQUEST, "filter 'property' is missing" + filterNode.asText());
            }

            JsonNode valueNode = filterNode.get(VALUE);
            if (valueNode == null) {
                throw new AgRESTException(Response.Status.BAD_REQUEST, "filter 'value' is missing" + filterNode.asText());
            }
